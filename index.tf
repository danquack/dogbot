...
resource "aws_api_gateway_resource" "index_resource" {
  path_part   = "prod"
...
resource "aws_api_gateway_integration" "index_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${data.aws_region.current.name}:lambda:path/2015-03-31/functions/${aws_lambda_function.index_lambda.arn}/invocations"
}

# Lambda
resource "aws_lambda_permission" "apigw_lambda_index" {
